...
def fetch_top_ads(limit=10, date_preset="last_7d", since=None, until=None):
    ...
    for i, insight in enumerate(all_insights):
        ...
        ad_params = {
            "access_token": META_ACCESS_TOKEN,
            "fields": "name,status,effective_status,creative{id,name,title,body,image_url,thumbnail_url,object_story_spec,asset_feed_spec,effective_object_story_id},adset{name,targeting}",
        }
...
def format_metrics_block(insights, batch_avgs=None):
    ...
    # Video metrics
    reach = insights.get("reach", "N/A")
    frequency = insights.get("frequency", "N/A")

    thruplay = "N/A"
    for vt in (insights.get("video_thruplay_watched_actions") or []):
        if vt.get("action_type") == "video_view":
            thruplay = vt.get("value", "N/A")
            break

    video_p25 = video_p50 = video_p75 = video_p100 = "N/A"
    for vp in (insights.get("video_p25_watched_actions") or []):
        if vp.get("action_type") == "video_view":
            video_p25 = vp.get("value", "N/A")
    for vp in (insights.get("video_p50_watched_actions") or []):
        if vp.get("action_type") == "video_view":
            video_p50 = vp.get("value", "N/A")
    for vp in (insights.get("video_p75_watched_actions") or []):
        if vp.get("action_type") == "video_view":
            video_p75 = vp.get("value", "N/A")
    for vp in (insights.get("video_p100_watched_actions") or []):
        if vp.get("action_type") == "video_view":
            video_p100 = vp.get("value", "N/A")

    video_avg_time = "N/A"
    for vat in (insights.get("video_avg_time_watched_actions") or []):
        if vat.get("action_type") == "video_view":
            video_avg_time = vat.get("value", "N/A")

    # Compute thumbstop rate (3-sec views / impressions)
    tsr = "N/A"
    tsr_rag = ""
    if thruplay != "N/A" and impressions != "N/A":
        try:
            tsr_val = float(thruplay) / float(impressions) * 100
            tsr = f"{tsr_val:.2f}"
            tsr_rag = rag_score(tsr, 30.0, 20.0, higher_is_better=True)
        except (ValueError, ZeroDivisionError):
            pass

    # Compute hold rates
    hold_15s = "N/A"
    if thruplay != "N/A" and video_p25 != "N/A":
        try:
            hold_15s_val = float(video_p25) / float(thruplay) * 100 if float(thruplay) > 0 else 0
            hold_15s = f"{hold_15s_val:.1f}"
        except (ValueError, ZeroDivisionError):
            pass
...
- Cost per Action: {cpa_str or 'N/A'}"""
    ...
    # Add video metrics block only if video data exists
    if thruplay != "N/A":
        block += f"""

## VIDEO METRICS
- ThruPlays (3s views): {thruplay}
- Thumbstop Rate (TSR): {tsr}% {tsr_rag}
- Watch 25%: {video_p25}
- Watch 50%: {video_p50}
- Watch 75%: {video_p75}
- Watch 100%: {video_p100}
- Hold Rate (25% / ThruPlay): {hold_15s}%
- Avg Watch Time: {video_avg_time}s"""
...
def compute_portfolio_aggregation(results):
    """Compute portfolio-level aggregations: spend concentration, format breakdown,
    funnel stage distribution, and landing page analysis."""

    total_spend = 0
    format_groups = {}  # format -> {spend, count, ctr_sum, ads}
    funnel_groups = {}  # awareness level -> {spend, count, ads}
    lp_groups = {}      # landing page URL -> {spend, count, ctr_sum, ads}
    spend_values = []
    ...
    for r in results:
        ...
        # Format grouping (from asset_type)
        fmt = r.get("asset_type", "unknown") or "unknown"
        if fmt not in format_groups:
            format_groups[fmt] = {"spend": 0, "count": 0, "ctr_sum": 0, "ads": []}
        format_groups[fmt]["spend"] += spend
        format_groups[fmt]["count"] += 1
        format_groups[fmt]["ctr_sum"] += ctr
        format_groups[fmt]["ads"].append(r.get("ad_name", "Unknown"))

        # Funnel stage grouping (from Gemini analysis text)
        analysis = r.get("analysis", "")
        funnel_stage = "Unclassified"
        for stage_label, keywords in [
            ("TOF (Problem-Unaware)", ["Problem-Unaware", "TOF", "Top of Funnel", "top-of-funnel"]),
            ("MOF (Problem-Aware)", ["Problem-Aware", "MOF", "Middle of Funnel", "mid-funnel"]),
            ("BOF (Solution-Aware)", ["Solution-Aware", "BOF", "Bottom of Funnel", "bottom-of-funnel"]),
        ]:
            if any(kw.lower() in analysis.lower() for kw in keywords):
                funnel_stage = stage_label
                break

        if funnel_stage not in funnel_groups:
            funnel_groups[funnel_stage] = {"spend": 0, "count": 0, "ads": []}
        funnel_groups[funnel_stage]["spend"] += spend
        funnel_groups[funnel_stage]["count"] += 1
        funnel_groups[funnel_stage]["ads"].append(r.get("ad_name", "Unknown"))

        # Landing page extraction from creative data
        creative = r.get("creative", {}) or {}
        oss = creative.get("object_story_spec", {}) or {}
        link_data = oss.get("link_data", {}) or {}
        lp_url = link_data.get("link", "")
        if not lp_url:
            video_data = oss.get("video_data", {}) or {}
            lp_url = video_data.get("call_to_action", {}).get("value", {}).get("link", "") if video_data else ""
        if not lp_url:
            afs = creative.get("asset_feed_spec", {}) or {}
            for link_url_obj in (afs.get("link_urls") or []):
                lp_url = link_url_obj.get("website_url", "")
                if lp_url:
                    break
        ...
        if lp_url:
            ...
            lp_key = f"{parsed.scheme}://{parsed.netloc}{parsed.path}"
        else:
            lp_key = "Unknown / DPA"

        if lp_key not in lp_groups:
            lp_groups[lp_key] = {"spend": 0, "count": 0, "ctr_sum": 0, "ads": []}
        lp_groups[lp_key]["spend"] += spend
        lp_groups[lp_key]["count"] += 1
        lp_groups[lp_key]["ctr_sum"] += ctr
        lp_groups[lp_key]["ads"].append(r.get("ad_name", "Unknown"))

    # Spend concentration
    spend_values.sort(reverse=True)
    n = len(spend_values)
    top3_spend = sum(spend_values[:3]) if n >= 3 else sum(spend_values)
    top5_spend = sum(spend_values[:5]) if n >= 5 else sum(spend_values)
    top10_spend = sum(spend_values[:10]) if n >= 10 else sum(spend_values)

    concentration = {
        "total_spend": f"{total_spend:.2f}",
        "total_ads": n,
        "top3_pct": f"{top3_spend/total_spend*100:.1f}%" if total_spend > 0 else "N/A",
        "top5_pct": f"{top5_spend/total_spend*100:.1f}%" if total_spend > 0 else "N/A",
        "top10_pct": f"{top10_spend/total_spend*100:.1f}%" if total_spend > 0 else "N/A",
        "top_ad_pct": f"{spend_values[0]/total_spend*100:.1f}%" if total_spend > 0 and n > 0 else "N/A",
    }

    # Format breakdown
    format_breakdown = []
    for fmt, data in sorted(format_groups.items(), key=lambda x: x[1]["spend"], reverse=True):
        avg_ctr = data["ctr_sum"] / data["count"] if data["count"] > 0 else 0
        format_breakdown.append({
            "format": fmt,
            "count": data["count"],
            "spend": f"{data['spend']:.2f}",
            "pct": f"{data['spend']/total_spend*100:.1f}%" if total_spend > 0 else "N/A",
            "avg_ctr": f"{avg_ctr:.2f}%",
        })

    # Funnel breakdown
    funnel_breakdown = []
    for stage, data in sorted(funnel_groups.items(), key=lambda x: x[1]["spend"], reverse=True):
    ...
    # LP breakdown
    lp_breakdown = []
    for lp, data in sorted(lp_groups.items(), key=lambda x: x[1]["spend"], reverse=True):
        avg_ctr = data["ctr_sum"] / data["count"] if data["count"] > 0 else 0
        lp_breakdown.append({
            "url": lp,
            "count": data["count"],
...
def generate_report(results, output_dir, analysis_type, batch_avgs, persona_synthesis=None,
    ...
    md.append(f"")
    md.append(f"| # | Ad Name | Spend | CTR | CTR RAG | CPC | CPM | TSR | TSR RAG | Media | Lifecycle |")
    md.append(f"|---|---------|-------|-----|---------|-----|-----|-----|---------|-------|-----------|")
    ...
    for i, r in enumerate(results):
        ...
        # Compute TSR for this ad
        ad_tsr = "—"
        ad_tsr_rag = ""
        ad_thruplay = None
        for vt in (ins.get("video_thruplay_watched_actions") or []):
            if vt.get("action_type") == "video_view":
                ad_thruplay = vt.get("value")
                break
        ad_impressions = ins.get("impressions", "0")
        if ad_thruplay and ad_impressions:
            try:
                tsr_v = float(ad_thruplay) / float(ad_impressions) * 100
                ad_tsr = f"{tsr_v:.1f}%"
                ad_tsr_rag = rag_score(tsr_v, 30.0, 20.0, higher_is_better=True)
            except (ValueError, ZeroDivisionError):
                pass
        ...
        md.append(
            ...
            f"${ins.get('cpm', 'N/A')} | {ad_tsr} | {ad_tsr_rag} | {media} | {lifecycle} |"
        )
    ...
    agg = compute_portfolio_aggregation(results)

    md.append(f"## Spend Concentration")
    md.append(f"")
    md.append(f"| Metric | Value |")
    md.append(f"|---|---|")
    md.append(f"| Total Spend | ${agg['concentration']['total_spend']} |")
    md.append(f"| Total Ads | {agg['concentration']['total_ads']} |")
    md.append(f"| Top Ad (% of total) | {agg['concentration']['top_ad_pct']} |")
    md.append(f"| Top 3 Ads (% of total) | {agg['concentration']['top3_pct']} |")
    md.append(f"| Top 5 Ads (% of total) | {agg['concentration']['top5_pct']} |")
    md.append(f"| Top 10 Ads (% of total) | {agg['concentration']['top10_pct']} |")
    md.append(f"")
    md.append(f"---")
    md.append(f"")

    md.append(f"## Format Breakdown")
    md.append(f"")
    md.append(f"| Format | # Ads | Spend | % of Total | Avg CTR |")
    md.append(f"|---|---|---|---|---|")
    for fb in agg["format_breakdown"]:
        md.append(f"| {fb['format']} | {fb['count']} | ${fb['spend']} | {fb['pct']} | {fb['avg_ctr']} |")
    md.append(f"")
    md.append(f"---")
    md.append(f"")

    md.append(f"## Funnel Stage Distribution")
    md.append(f"")
    md.append(f"| Stage | # Ads | Spend | % of Total |")
    md.append(f"|---|---|---|---|")
    for fs in agg["funnel_breakdown"]:
        md.append(f"| {fs['stage']} | {fs['count']} | ${fs['spend']} | {fs['pct']} |")
    md.append(f"")
    md.append(f"---")
    md.append(f"")

    md.append(f"## Landing Page Analysis")
    md.append(f"")
    md.append(f"| Landing Page | # Ads | Spend | % of Total | Avg CTR |")
    md.append(f"|---|---|---|---|---|")
    for lp in agg["lp_breakdown"]:
        url_display = lp['url'][:60] + "..." if len(lp['url']) > 60 else lp['url']
        md.append(f"| {url_display} | {lp['count']} | ${lp['spend']} | {lp['pct']} | {lp['avg_ctr']} |")
    md.append(f"")
    md.append(f"---")
    md.append(f"")
